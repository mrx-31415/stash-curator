...
Expand refresh is incremental: it fetches only StashDB entries changed since the last
refresh, keeps the existing candidate pool and explore rows from hunts and StashDB
Similar, re-scores the pool when the model has changed, and drops candidates older than
the recent-release horizon.
