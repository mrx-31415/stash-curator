...
def _pair_score(
    ...
    coverage = sum(context.rarity(t) for t in tags_a ^ tags_b)
    perfs_a = context.scene_performers.get(a, frozenset())
    perfs_b = context.scene_performers.get(b, frozenset())
    coverage += sum(_pair_rarity(context, p) for p in perfs_a ^ perfs_b)
    if dimension == "tag":
...
def _orthogonal_pairs(
    ...
    """Coverage-ranked adjacency pairing: 2x budget scenes, adjacent pairs."""
    unlabeled = _pair_unlabeled(context, seen)
    unlabeled.sort(key=lambda sid: (-_scene_coverage(context, sid), sid))
    take = min(len(unlabeled), 2 * budget)
    scenes = unlabeled[:take]
    return [(scenes[i], scenes[i + 1]) for i in range(0, len(scenes) - 1, 2)][:budget]
...
def _update_elo(
    connection: sqlite3.Connection, scene_a: str, scene_b: str, winner: str, now_ms: int
) -> None:
    def current(scene_id: str) -> float:
        row = connection.execute(
            "SELECT elo FROM curation_pair_elo WHERE scene_id=?", (scene_id,)
        ).fetchone()
        return float(row["elo"]) if row else PAIR_ELO_INITIAL

    ra, rb = current(scene_a), current(scene_b)
    ea = 1.0 / (1.0 + 10.0 ** ((rb - ra) / 400.0))
    eb = 1.0 - ea
    if winner == "a":
        ra += PAIR_ELO_K * (1.0 - ea)
        rb += PAIR_ELO_K * (0.0 - eb)
    else:
        ra += PAIR_ELO_K * (0.0 - ea)
        rb += PAIR_ELO_K * (1.0 - eb)
    for scene_id, elo in ((scene_a, ra), (scene_b, rb)):
        connection.execute(
            """
            INSERT INTO curation_pair_elo(scene_id, elo, updated_at_ms)
            VALUES (?, ?, ?)
            ON CONFLICT(scene_id) DO UPDATE SET
                elo=excluded.elo, updated_at_ms=excluded.updated_at_ms
            """,
            (scene_id, elo, now_ms),
        )
