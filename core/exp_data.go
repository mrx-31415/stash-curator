...
// exp data from glibc's e_exp_data.c (N = 128, EXP_TABLE_BITS = 7,
// EXP_POLY_ORDER = 5). The hex literals carry over verbatim: integer hex is
// valid Go, and Go's hex float literals match the C hex floats exactly.
const expN = 128

const expTableBits = 7

const expInvLn2N = 0x1.71547652b82fep0 * 128 // N/ln2

const expNegLn2hiN = -0x1.62e42fefa0000p-8 // -ln2/N, high part

const expNegLn2loN = -0x1.cf79abc9e3b3ap-47 // -ln2/N, low part

const expShift = 0x1.8p52 // rounding shift for the non-intrinsic path

// expPoly are the C2..C5 quartic coefficients (ulp error 0.509 with fma).
var expPoly = [4]float64{
