-- Stage 2 of the lane redesign (docs/workpackage-lane-redesign.md): Adventure
-- is replaced by Blind Spots. Same shape as migration 0034 (Stretch): every
-- lane/source_lane CHECK constraint naming 'adventure' cannot be altered in
-- place, so each table carrying one is rebuilt again: model_scene_lane,
-- model_lane_candidate_cache, and model_lane_order + model_lane_order_state.
--
-- No data migration: Blind Spots' qualification_json shape (dark_facets,
-- corroborating_types) has nothing in common with Adventure's, so every row
-- in these materialized caches is stale regardless of the rename and is
-- dropped rather than remapped.
--
-- main-qualified and shadow-view handling: see migration 0034 for the full
-- explanation (MODEL_TABLES entries are shadowed by a temp view whenever a
-- model artifact is attached; DROP/CREATE TABLE targets main. directly,
-- and model_scene_lane's two indexes — which CREATE INDEX cannot
-- schema-qualify — are preceded by dropping that shadow view outright).
...
CREATE TABLE main.model_lane_order (
    ...
    lane TEXT NOT NULL CHECK (
        lane IN ('for_you', 'best_bets', 'revisit', 'stretch', 'blind_spots')
    ),
...
-- unused; this rebuild carries the same access pattern as 0034 and stays
-- without it.
