(function () {
  ...
  const ExternalCard = Api.register.component("stash-curator.ExternalCard", function ExternalCard(props) {
    ...
    const localProfile = kind === "performer" && payload.curator_local ? { to: `/performers/${payload.curator_local.id}`, title: "Open local performer profile" } : { href, target: "_blank", rel: "noreferrer" };
    ...
    return React.createElement(
      "article",
      { className: `curator-card curator-external-card curator-external-${kind} grid-card ${kind}-card${kind === "performer" && payload.curator_local ? " curator-external-local" : ""}` },
      ...
      kind === "performer" && payload.curator_local && React.createElement("span", { className: "curator-local-badge", title: "Already in your local library" }, "In library"),
      ...
      React.createElement("div", { className: "curator-card-body" }, (() => { let scoreDetail; if (item.similarity === undefined) { scoreDetail = `Match ${item.score.toFixed(2)} · found via ${item.sources.join(", ")}`; } else { scoreDetail = `Similarity ${item.similarity.toFixed(2)} · appeal ${item.appeal.toFixed(2)}`; const mh = item.details && item.details.score_breakdown && item.details.score_breakdown.multi_hop; if (mh > 0) scoreDetail += " + multi-hop " + mh.toFixed(4); } return React.createElement("div", { className: "curator-card-details" }, payload.why?.length && React.createElement("details", { className: "curator-evidence" }, React.createElement("summary", null, "Why this?"), React.createElement("p", { className: "curator-explanation" }, payload.why.join(" · "))), React.createElement("details", { className: "curator-score" }, React.createElement("summary", null, `Score · ${item.score.toFixed(2)}`), React.createElement("p", null, scoreDetail))); })()),
  ...
  function SimilarityPanel({ initialType = "scene", initialId = null, initialLabel = null }) {
    ...
    function load(id, label, type = entityType, nextSource = source, nextGender = gender, nextPage = 1, nextExcluded = excludedIds, nextIncludeOwned = includeOwned) {
      ...
      if (nextSource === "stashdb" && type === "performer") request.include_owned = nextIncludeOwned;
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        entityType === "performer" && source === "stashdb" && React.createElement(Button, { className: "curator-include-owned", size: "sm", variant: includeOwned ? "primary" : "secondary", "aria-pressed": includeOwned, title: "Include performers already in your library so the remote ranking can be compared with the local search", "aria-label": includeOwned ? "Hide library performers" : "Include library performers", onClick: () => { const next = !includeOwned; setIncludeOwned(next); if (selected) load(selected.id, selected.label, entityType, source, gender, 1, [], next); } }, React.createElement(FontAwesomeIcon, { icon: faUserCheck }), " Local"),
