...
def _apply_plugin_settings(connection: Any, settings: dict[str, Any]) -> None:
    mapping = {
        ...
        "scheduleExpandRefreshEnabled": ("schedule_expand_refresh_enabled", bool),
        "scheduleExpandRefreshIntervalHours": ("schedule_expand_refresh_interval_hours", float),
        "scheduleExpandRefreshAtHour": ("schedule_expand_refresh_at_hour", int),
        "scheduleSyncBuildEnabled": ("schedule_sync_build_enabled", bool),
        "scheduleSyncBuildIntervalHours": ("schedule_sync_build_interval_hours", float),
        "scheduleSyncBuildAtHour": ("schedule_sync_build_at_hour", int),
        "scheduleBackupEnabled": ("schedule_backup_enabled", bool),
        "scheduleBackupIntervalHours": ("schedule_backup_interval_hours", float),
        "scheduleBackupAtHour": ("schedule_backup_at_hour", int),
    ...
    try:
        ...
        if operation == "get_curation_picks":
            return api.curation_picks(
                ...
                int(args.get("budget") or 10),
                str(args.get("base_tag_id") or "") or None,
                str(args.get("context_tag_id") or "") or None,
                str(args.get("performer_id") or "") or None,
            )
        if operation == "submit_curation_picks":
